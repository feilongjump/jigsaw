...
import { Input, Modal, ModalContent, ModalHeader, ModalBody, ModalFooter, Button, useDisclosure } from "@heroui/react";
        ...
        <div className="relative flex flex-col gap-4 group px-2">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    {note.updated_at && <span className="text-xs text-[#b2bec3]">更新于 {fromNow(note.updated_at)}</span>}
                    {(note.tags && note.tags.length > 0) && (
                        <div className="flex gap-2">
                            {note.tags.map((tag: string, idx: number) => (
                                <span key={idx} className="text-[#0984E3] bg-[#0984E3]/10 px-2 py-0.5 rounded-md text-xs whitespace-nowrap">#{tag}</span>
                            ))}
                        </div>
                    )}
                </div>
            ...
            {showExpandBtn && (
                <div className="flex justify-center mt-2">
                    ...
                    >
                        {isExpanded ? (
                            <>
                                收起 <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="18 15 12 9 6 15"></polyline></svg>
                            </>
                        ) : (
                            <>
                                展开全文 <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg>
                            </>
                        )}
                    </button>
                </div>
...
function NotesPage() {
  ...
  const getGreeting = () => {
    ...
    if (hour < 5) return "凌晨了，要注意休息 🌙";
    if (hour < 11) return "早上好啊！新的一天，新的快乐！ ☀️";
    if (hour < 13) return "中午好，有空要小憩一会 ☕";
    if (hour < 19) return "下午好，饿了就吃点东西垫巴垫巴 🍪";
    return "晚上好，准备睡觉咯 🛌";
  };
      ...
      <main className="flex-1 overflow-y-auto px-4 pt-2 pb-24 no-scrollbar">
        ...
        <div className="mb-5">
            <Input 
                ...
                startContent={
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-gray-400"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>
                }
                classNames={{
                    base: "w-full",
                    mainWrapper: "h-full",
